...
ANIM_OUT="data/avatars/arab-man/_${WORD}_v2_anim.glb"
MERGED_OUT="data/avatars/arab-man/arab_sheik_${WORD}_v2.glb"
VIDEO_OUT="/root/.openclaw/workspace/arab_sheik_${WORD}_v2.mp4"
...
if [ ! -f "$HOLISTIC_JSON" ]; then
  echo "=== 1/4 Extracting MediaPipe Holistic ==="
  python3 scripts/animate/extract_holistic.py "$SOURCE_VIDEO" "$HOLISTIC_JSON"
fi

echo ""
echo "=== 2/4 Retargeting v2 (hierarchical, avatar-aware) ==="
python3 scripts/animate/retarget_v2.py "$AVATAR" "$HOLISTIC_JSON" "$ANIM_OUT" --smooth 5 --trim-trailing
